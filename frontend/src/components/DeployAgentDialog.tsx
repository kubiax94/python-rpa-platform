...
import { prepareDeployment, useDeployment } from "@/hooks/useDeploymentAPI";
...
export function DeployAgentDialog({ open, initialAgentId, initialHostname, initialDisplayName, onClose }: DeployAgentDialogProps) {
  ...
  }, [open, initialAgentId, initialDisplayName, initialHostname]);
            ...
            {deployment && (
              <div className="mt-4 space-y-3 text-sm">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${deployment.status === "ready" ? "bg-emerald-500/15 text-emerald-300" : deployment.status === "failed" ? "bg-red-500/15 text-red-300" : "bg-amber-500/15 text-amber-300"}`}>
                    {deployment.status}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-3">
